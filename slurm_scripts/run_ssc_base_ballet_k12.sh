#!/bin/bash
#SBATCH --job-name=ssc_base_ballet_k12
#SBATCH --account=minjilab99
#SBATCH --partition=standard
#SBATCH --time=1-00:00:00
#SBATCH --mem=32G
#SBATCH --cpus-per-task=4
#SBATCH --output=/nfs/turbo/umms-minjilab/lpullela/icassp_ssc_tv2/ballet_dataset/logs/ssc_base_ballet_k12_%j.out
#SBATCH --error=/nfs/turbo/umms-minjilab/lpullela/icassp_ssc_tv2/ballet_dataset/logs/ssc_base_ballet_k12_%j.err

set -euo pipefail

# Load lmod if available
if [ -f /etc/profile.d/lmod.sh ]; then . /etc/profile.d/lmod.sh; fi
if [ -f /sw/lmod/lmod/init/bash ]; then . /sw/lmod/lmod/init/bash; fi

# Activate conda environment
source /home/lpullela/miniconda3/etc/profile.d/conda.sh
conda activate ssc_559

# Set threading environment variables
export OMP_NUM_THREADS=${SLURM_CPUS_PER_TASK:-4}
export MKL_NUM_THREADS=${SLURM_CPUS_PER_TASK:-4}
export OPENBLAS_NUM_THREADS=${SLURM_CPUS_PER_TASK:-4}
export NUMEXPR_NUM_THREADS=${SLURM_CPUS_PER_TASK:-4}
export PYTHONUNBUFFERED=1

echo "========================================"
echo "Job: ssc_base_ballet_k12"
echo "Dataset: ballet"
echo "K: 12"
echo "known_k: True"
echo "extra_args: (none)"
echo "Methods: OSC TKSS BDOSC"
echo "========================================"
echo "host=$(hostname)  python=$(which python)  start=$(date)"

cd /nfs/turbo/umms-minjilab/lpullela/icassp_ssc_tv2/ballet_dataset

# Create logs directory if it doesn't exist
mkdir -p logs

python -u cluster_experiment.py \
    --k 12 \
    --known-k \
    --methods OSC TKSS BDOSC \
    --n-trials 100 \
    --out-tag k12_baselines

echo "end=$(date)"
echo "Experiment completed!"
