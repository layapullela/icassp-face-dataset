        ...
        for k in range(K):
            ...
            if np.sum(w_k) > 0:
                ...
                if n_keep <= 0:
                    ...
                    # Only the top n_keep singular vectors are ever used, so a
                    # truncated SVD is enough -- avoids paying for the full
                    # min(D, N)-rank decomposition (the dominant cost once N
                    # grows with K, e.g. surveillance k=8).
