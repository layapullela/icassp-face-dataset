#!/usr/bin/env python3
"""Summarize the ballet hold-out CV benchmark: table + figures.

Mirrors summarize_cv.py (surveillance), reading every results CSV under
cv/results/ballet/ (one per method x k x sigma) plus the matching
*_timing.json inference-time summaries, and producing:

  cv/analysis/ballet_summary_table.csv / .md   -- test/train ARI,NMI,ACC
      per (method, k, sigma), mean +/- std across CV folds
  cv/analysis/ballet_inference_time.csv        -- mean inference seconds
      per (method, k), pooled across sigma
  cv/analysis/figures/ballet_metrics_sigma{s}.png  -- ARI/NMI/ACC bar
      charts, one figure per sigma, grouped by k, error bars = std
      across folds
  cv/analysis/figures/ballet_inference_time.png    -- mean inference
      time per method, faceted by k (log-scale y)

k=3,5,8, same K_VALUES as surveillance, but fold counts differ: 44
ballet sequences shuffled and cut into equal groups of k, remainder
dropped -- k=3: 14 folds, k=5: 8 folds, k=8: 5 folds (see
block_ssc_benchmark/README.md, "Hold-out CV benchmark (ballet)"). Every
tuned method (SSC-Block-TV-Col21, OSC, TKSS) uses --tune-pool-groups 20,
same as surveillance.

Unlike surveillance (24 camera-view matrices per held-out people-group),
each ballet fold's test set is a single matrix (one k-sequence group),
so per-fold ARI here is a noisier estimate than surveillance's -- keep
that in mind when comparing error bars across the two figures.
"""
...
def write_summary_table(summary: dict) -> tuple[Path, Path]:
    csv_path = OUT / "ballet_summary_table.csv"
    md_path = OUT / "ballet_summary_table.md"
    header = [
    ...
    with md_path.open("w") as f:
        f.write("# Ballet hold-out CV summary\n\n")
        f.write(
            "Mean +/- std across CV folds (fold = one held-out group of k "
            "sequences). k=3: 14 folds, k=5: 8 folds, k=8: 5 folds. Tuned "
            "methods (SSC-Block-TV-Col21, OSC, TKSS) use "
            "--tune-pool-groups 20 (see module docstring). Unlike "
            "surveillance, each fold's test set is a single matrix, so "
            "per-fold estimates here are noisier.\n\n"
        )
...
def write_timing_table(timing: dict) -> Path:
    path = OUT / "ballet_inference_time.csv"
...
def plot_metrics(summary: dict) -> list[Path]:
    ...
    for sigma in SIGMAS:
        ...
        fig.suptitle(
            f"Ballet hold-out CV — test-set, known k, σ={sigma:g}  "
            "(error bars: std across CV folds)",
            fontsize=13,
        )
        path = FIGURES / f"ballet_metrics_sigma{sigma_tag(sigma)}.png"
...
def plot_timing(timing: dict) -> Path:
    ...
    fig.suptitle(
        "Ballet hold-out CV — inference time per method "
        "(pooled across σ, excludes tuning time)",
        fontsize=13,
    )
    path = FIGURES / "ballet_inference_time.png"
    fig.savefig(path, dpi=180)
    plt.close(fig)
    return path
