...
SSC_DEFAULTS = dict(lambda_e=1.0, lambda_z=0.1, gamma=0.1)
...
def run_ssc_tv(Y, k, lambda_e, lambda_z, gamma):
    X, _, _ = ssc_admm_nuc_tv(
        Y, lambda_e=lambda_e, lambda_z=lambda_z, gamma=gamma, max_iter=50,
    )
...
def build_split_mats(loaded, train_groups, test_group, sigma, rng):
    ...
    for ds_name, Y01, labels, person_dirs, paths in loaded:
        ...
        Y = column_normalize(Y01 + sigma * noise)
...
def main():
    ...
    for name in selected:
        ...
        print(f"\n{'=' * 60}")
        print(f"--- tune {name} over all σ={SIGMAS} ---")
        result = tune_over(
