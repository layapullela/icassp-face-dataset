#!/bin/bash
#SBATCH --job-name=surv_bcmp
#SBATCH --time=1-00:00:00
#SBATCH --mem=64G
#SBATCH --cpus-per-task=4
#SBATCH --partition=standard
#SBATCH --account=minjilab99
#SBATCH --output=/nfs/turbo/umms-minjilab/lpullela/icassp_ssc_tv2/surveillance_dataset/logs/cluster_bcmp_%j.out
#SBATCH --error=/nfs/turbo/umms-minjilab/lpullela/icassp_ssc_tv2/surveillance_dataset/logs/cluster_bcmp_%j.err
#
...
if [ "$MODE" = "hetero" ]; then
    python -u "$ROOT/cluster_experiment.py" \
        --hetero-noise --k "$K" --n-trials "$N_TRIALS" --known-k \
        --out-tag "${TAG_PREFIX}het" \
        --methods "${METHODS[@]}" \
        "${BLOCK_ARGS[@]}"
elif [ "$MODE" = "clean" ]; then
    python -u "$ROOT/cluster_experiment.py" \
        --sigmas 0.0 --k "$K" --n-trials "$N_TRIALS" --known-k \
